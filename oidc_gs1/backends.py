...
class OIDCAuthBackend(ModelBackend):
    ...
    def authenticate(self, nonce, request):
        ...
        if 'http://gs1.org/claims/crm-Email' not in userinfo_response_data:
            return

        # Tries to retrieve a corresponding user in the local database and creates it if applicable.
        try:
            oidc_user = OIDCUser.objects.select_related('user').get(sub=userinfo_response_data.get('sub'))
            no_user = False
...
def create_oidc_user_from_claims(claims):
    ...
    sub = claims['sub']
    email = claims['http://gs1.org/claims/crm-Email']
...
def update_oidc_user_from_claims(oidc_user, claims):
    ...
    oidc_user.save()
    oidc_user.user.email = claims['http://gs1.org/claims/crm-Email']
    oidc_user.user.save()
