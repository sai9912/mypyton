...
class ServiceManager(models.Manager):
    ...
    def get_form_choices(self):
        choices = [(row.code, row.uom) for row in DimensionUOM.objects.all()]
        choices.insert(0, ('', ''))
        return choices

    def get_id_from_code(self, code):
        try:
            item = DimensionUOM.objects.get(code=code)
        except Exception as e:
            return None
        return item.id


class DimensionUOM(models.Model, TranslatedFieldsMixin):
